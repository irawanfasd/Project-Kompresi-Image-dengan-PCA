...
@app.route('/test/input/<filename>')
def serve_input(filename):
    return send_from_directory(app.config['UPLOAD_FOLDER'], filename)

@app.route('/test/output/<filename>')
...
def compress_api():
    ...
    if k_value < 1:
        return jsonify({'error': 'Nilai K harus lebih besar dari 0'}), 400

    if not (file and file_diizinkan(file.filename)):
    ...
    # nama file output sementara (akan disesuaikan jadi .jpg oleh compress_image)
    output_filename = f"compressed_{k_value}_{filename}"
    output_path = os.path.join(app.config['OUTPUT_FOLDER'], output_filename)

    # jalankan kompresi dan catat waktu
    start_time = time.time()
    result = compress_image(input_path, output_path, k_value)
    end_time = time.time()

    if not result:
        return jsonify({'error': 'Gagal melakukan kompresi gambar! Cek flask.log untuk detail.'}), 500

    # compress_image mengembalikan path file JPEG aktual yang dipakai
    output_path = result
...
if __name__ == '__main__':
    # debug=False untuk production: mencegah exposure Werkzeug debugger
    # ke publik dan mengurangi risiko proses restart ganda/nyangkut.
